...
int main()
    {
        ...
        const int numMeasurements = 10;
            ...
            for (int numThreads = 1; numThreads <= maxThreads; numThreads ++)
            {
                ...
                {
                    // Initialize arrays a[] and b[]. N*8 bytes (~800MB) is far too large for the
                    // stack (a plain `double a[N], b[N]` here segfaults immediately on any
                    // normal stack-size limit) -- heap-allocate instead.
