...
{
    int size = 12000, tilesize = 3000, xtiles, ytiles, xc, yc, count, tilebase, i;
    char* tile, * picture;
    double ws, we, cs, ce;

    // Run the loop from 1 to 72 threads

    for (int num_threads = 1; num_threads <= 72; num_threads++)
    {
...
#pragma omp parallel private(tile)
        {
            ...
            count = 0;
#pragma omp for private(xc,i) schedule(dynamic) collapse(2)
        ...
        we = getTimeStamp();
        double perf = (size * size) / ((we - ws) * 1e6);
        fprintf(stderr, "Threads: %d\t Time: %f\t Perf: %f\n", num_threads, we - ws, perf);
