...
import numpy as np
from .projectedInexactNewtonCG import projectedInexactNewtonCG as PCG
from .augmentedLagrangianObjective import augmentedLagrangianObjective as AO
...
def augmentedLagrangianDescent(f, P, h, x0: np.array, alpha0=0, eps=1.0e-3, delta=1.0e-6, verbose=0):
    ...
    xp = P.project(x0)
    myAugmentedObjective = AO(f, h, alphak, gammak)

    while True:
        # Minimize Ak(x) subject to x ∈ Ω□
        xp = PCG(myAugmentedObjective, P, xp, epsk)

        # Check stationarity and feasibility
