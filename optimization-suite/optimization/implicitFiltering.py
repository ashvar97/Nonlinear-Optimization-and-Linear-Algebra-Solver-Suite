...
def derivativefreeBFGSDescent(f, x0: np.array, h, eps=1.0e-3, sigma=1.0e-4, verbose=0):
    ...
    x = x0
    grad_f_h = SUCSimplexGradient(f, x, h)
    n = x0.shape[0]
    E = np.eye(n)
    B = E

    isStencilFailure = SUCStencilFailure(f, x, h)
    loopCounter = 0
    linesearchFail = 0
    ...
    while not satisfiesTermination:
        ...
        if linesearchFail:
            ...
            x_new = x + t * d
            grad_f_h_new = SUCSimplexGradient(f, x_new, h)
            ...
            s_k = delta_xk
            y_k = delta_gk
            curvature = y_k.T @ s_k  # Curvature condition: must be bounded away from 0

            if np.abs(curvature) > 1.0e-10:
                rho_k = 1 / curvature
                B = (E - rho_k * s_k @ y_k.T) @ B @ (E - rho_k * y_k @ s_k.T) + rho_k * s_k @ s_k.T
            elif verbose:
                print('Skipping BFGS update: curvature condition y_k.T@s_k is too close to 0.')
