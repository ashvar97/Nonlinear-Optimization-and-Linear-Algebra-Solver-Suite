...
def projectedBFGSDescent(f, P, x0: np.array, eps=1.0e-3, verbose=0):
    ...
    maxIter = 500  # guards against pathologically slow convergence (e.g. a badly-conditioned
                   # objective near a box edge, where BFGS can degrade towards steepest descent)
    while np.linalg.norm(xp - P.project(xp - gradx)) > eps and countIter < maxIter:
        ...
        t = PB(f, P, xp, d)  # Compute the step size
        ...
        if np.abs(curvature) > 1.0e-10:
            rho = 1 / curvature  # Compute the scaling factor
    ...
    if verbose:
        if countIter >= maxIter:
            print('Warning: projectedBFGSDescent reached the iteration limit before converging.')
